...
# allow: python .\scripts\eval_on_gold_dcn_v1.py
...
def main():
    ...
    # config params (must match training)
    if cfg is None:
    ...
    print("embed_dim:", embed_dim, "deep:", hidden_dims, "cross_layers:", cross_layers)

    metrics = eval_model(model, gold_loader, device=device, max_batches=args.max_batches)
    print("\n===== GOLD EVAL METRICS (DCN v1) =====")
