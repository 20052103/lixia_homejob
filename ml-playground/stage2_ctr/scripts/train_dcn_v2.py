...
def main():
    ...
    cross_rank = int(cfg.get("dcnv2", {}).get("rank", 32))
    cross_experts = int(cfg.get("dcnv2", {}).get("experts", 4))
    ...
    # prior bias (same as other models)
    ...
    print("dcnv2: layers=", cross_layers, "rank=", cross_rank, "experts=", cross_experts)

    feature_artifacts = build_feature_artifacts(hash_bins=hash_bins, seed=seed, dense_transform=dense_transform)
    ...
    run_name = args.run_name or time.strftime("dcn_v2_%Y%m%d_%H%M%S")
    metrics = {
        ...
        "dcnv2_cross_layers": cross_layers,
        "dcnv2_rank": cross_rank,
        "dcnv2_experts": cross_experts,
        "p_prior": p,
    }
