...
from stage3.models.autoint import AutoIntCTR
...
def eval_logloss(model, loader, device, max_batches=200, hash_bins=None):
    ...
    for i, (dense, sparse, y) in enumerate(loader):
        ...
        # Convert global IDs -> per-field local IDs if needed
...
def save_run(model, cfg, metrics: dict, out_root=r"D:\repo\large data\model"):
    try:
        ...
        run_dir = os.path.join(out_root, f"dnn_autoint_{run_id}")
        os.makedirs(run_dir, exist_ok=True)

        # 1) model weights
        torch.save(model.state_dict(), os.path.join(run_dir, "model.pt"))
        print(f"✅ saved model to: {os.path.join(run_dir, 'model.pt')}")
...
def build_optimizers_embedding_sgd_other_adamw(
    ...
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        lname = name.lower()
        # Match our tokenizer naming: cat_emb (shared) or cat_embs.* (per-field)
        if ("cat_emb" in lname) or ("cat_embs" in lname) or (".emb" in lname) or ("embedding" in lname):
...
def main():
    ...
    num_cat = 26
    hash_bins = int(cfg["preprocess"]["hash_bins"])
    cat_cardinalities = [hash_bins] * num_cat
    ...
    for ep in range(1, epochs + 1):
        ...
        for step, (dense, sparse, y) in enumerate(train_loader, start=1):
            ...
            # Stage2 sparse are global IDs with offsets -> local per-field ID
            sparse = (sparse % hash_bins).long()
        ...
        # Save checkpoint
        ckpt_path = os.path.join(ckpt_dir, f"autoint_ep{ep}.pt")
        torch.save({"model": model.state_dict(), "cfg": cfg}, ckpt_path)
        print(f"[epoch {ep}] saved={ckpt_path} epoch_time={time.time()-t0:.1f}s")

        # Quick eval (logloss only, subset)
